...
protected:
    ...
    /** Fetches the neighbors for the slot at the specified coordinates
      ...
      */
    std::vector<int> getNeighborIndices( const PointPlus<int>& pt )
    {
        ASSERT_MSG( pt.x >= 0 && pt.x < _sz.w, "x coordinate must be in the range [ 0 , w )" );
        ASSERT_MSG( pt.y >= 0 && pt.y < _sz.h, "y coordinate must be in the range [ 0 , h )" );
        vector<int> indices;                                                                                            // Will contain the indices of all neighbor cells
        int o = ( pt.y + 1 ) % 2;                                                                                       // An offset for rows.  If this row is odd ( shorter by 1 ) the offset is 1.  Alleviates problem with staggering;
        indices.push_back( getIndex( PointPlus<int>( mod(   pt.x-1, _sz.w ),                 pt.y ) ) );                // Neighbor to the left.  Notice that we use correct modular arithmethic to wrap around edges.  The % operator will not perform correctly here
        indices.push_back( getIndex( PointPlus<int>( mod(   pt.x+1, _sz.w ),                 pt.y ) ) );                // Neighbor to the right
        indices.push_back( getIndex( PointPlus<int>( mod(   pt.x-o, _sz.w ), mod( pt.y-1, _sz.h ) ) ) );                // Neighbor to the up-left
        indices.push_back( getIndex( PointPlus<int>( mod( pt.x+1-o, _sz.w ), mod( pt.y-1, _sz.h ) ) ) );                // Neighbor to the up-right
        indices.push_back( getIndex( PointPlus<int>( mod(   pt.x-o, _sz.w ), mod( pt.y+1, _sz.h ) ) ) );                // Neighbor to the down-left
        indices.push_back( getIndex( PointPlus<int>( mod( pt.x+1-o, _sz.w ), mod( pt.y+1, _sz.h ) ) ) );                // Neighbor to the down-right
        return indices;
    }
    ...
    /** Fetches the neighbors for the specified slot
      ...
      *         wrapping is not available
      */
    std::vector<int> getNeighborIndices( int idx )
    {
        return getNeighborIndices( getCoords( idx ) );
    }
    ...
    /** Fetches indices for all slots within a radius of the slot at x, y
      ...
      */
    std::vector<int> getNeighborhood( double r, const PointPlus<double>& pt )
    {
        return getNeighborhood( r, getIndex(pt) );
    }
...
public:
    ...
    /** Fetches the item at the given coordinates
      * @param  pt - The point coordinate of the desired slot
      * @return A reference to the desired item
      */
    T& fetchItem( const PointPlus<int>& pt )
    {
